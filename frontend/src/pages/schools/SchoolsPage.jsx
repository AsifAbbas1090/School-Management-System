...
const SchoolsPage = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        // Handle logo upload (in production, upload to Firebase Storage)
        ...
        if (formData.logoFile) {
            ...
            logoUrl = editingSchool.logoUrl || editingSchool.logo || formData.logo;
        }
        ...
        if (formData.subscriptionStartDate) {
            schoolData.subscriptionStartDate = new Date(formData.subscriptionStartDate).toISOString();
        }

        // Admin user credentials (if creating new school)
        if (!editingSchool) {
            if (formData.adminEmail) schoolData.adminEmail = formData.adminEmail;
            if (formData.adminPassword) schoolData.adminPassword = formData.adminPassword;
        }

        // Admin user creation is handled by backend when school is created
        // The adminEmail and adminPassword are sent to backend in schoolData

        try {
            if (editingSchool) {
                // Update via API
                const response = await schoolsService.update(editingSchool.id, schoolData);
                if (response.success && response.data) {
                    updateSchool(editingSchool.id, response.data);
                    toast.success('School updated successfully!');
                    resetForm();
                } else {
                    toast.error(response.error || 'Failed to update school');
                    return;
                }
            } else {
                // Create via API
                const response = await schoolsService.create(schoolData);
                if (response.success && response.data) {
                    ...
                    toast.error(response.error || 'Failed to create school');
                }
            }
        } catch (error) {
    ...
    const handleDelete = async (schoolId) => {
        if (window.confirm('Are you sure you want to delete this school? This action cannot be undone.')) {
            const response = await schoolsService.delete(schoolId);
            if (response.success) {
                deleteSchool(schoolId);
                toast.success('School deleted successfully!');
            } else {
                toast.error(response.error || 'Failed to delete school');
            }
        }
    };
